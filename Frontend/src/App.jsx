...
function App() {
  ...
  const [payloadGen, setPayloadGen] = useState(
    '{"sub":"123456","name":"Ejemplo","iat":1699000000}'
  );
  ...
  return (
    <div
      ...
      <section style={{ marginBottom: 38 }}>
        <h2 style={{ marginBottom: 12, color: "#313cff" }}>
        ...
        {result && (
          <div style={{ marginTop: 28 }}>
            <div>
              <span style={{ fontWeight: 600, color: "#1b8700" }}>
                ✓ Estructura:{" "}
              </span>
              {result.estructura_valida ? (
                "Válida"
              ) : (
                <span style={{ color: "#ba0034" }}>Inválida</span>
              )}
            </div>
            <div style={{ marginBottom: 10 }}>
              <strong>Header:</strong>
              <pre>{pretty(result.header)}</pre>
            </div>
            <div style={{ marginBottom: 10 }}>
              <strong>Payload:</strong>
              <pre>{pretty(result.payload)}</pre>
            </div>
            <div style={{ marginBottom: 10 }}>
              ...
              <ul>
                {result.payload &&
                  ...
                  Object.keys(result.payload).map((c, i) => (
                    <li key={i}>
                      <b>{c}</b>: {JSON.stringify(result.payload[c])}
                    </li>
                  ))}
              </ul>
            </div>
            ...
            <div style={{ marginBottom: 10, fontWeight: 500 }}>
              ...
              {typeof result.firma_valida !== "undefined" ? (
                result.firma_valida === true ? (
                  ...
                  <span style={{ color: "#ba0034" }}>
                    {result.firma_valida === false
                      ? "✗ Inválida"
                      : result.firma_valida}
                  </span>
                )
      ...
      <section style={{ marginTop: 38 }}>
        ...
        {history.length === 0 && !historyLoading && !historyError && (
          <p>No hay historial de análisis aún.</p>
        )}
        {history.length > 0 && (
          <div
            style={{
            ...
            {history.map((record, index) => (
              <div
                ...
                <div style={{ marginBottom: 5 }}>
                  <strong>Header:</strong>{" "}
                  <pre>{pretty(record.analysis_result.header)}</pre>
                </div>
                <div style={{ marginBottom: 5 }}>
                  <strong>Payload:</strong>{" "}
                  <pre>{pretty(record.analysis_result.payload)}</pre>
                </div>
                <div style={{ marginBottom: 5 }}>
                ...
                <div style={{ marginBottom: 5 }}>
                  ...
                  {typeof record.analysis_result.firma_valida !== "undefined"
                    ? record.analysis_result.firma_valida === true
                      ? "Válida"
                      : record.analysis_result.firma_valida === false
                      ? "Inválida"
                      : record.analysis_result.firma_valida
                    : "No verificada"}
                </div>
              </div>
            ))}
      ...
      <div style={{ marginTop: 38, fontSize: "14px", color: "#777" }}>
        <b>Pautas validadas:</b> Estructura, decodificación Base64URL, parsing
        JSON, visualización de claims, validación semántica, firma HS256/HS384,
